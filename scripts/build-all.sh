...
if [[ "$1" = "release" ]] ; then
	...
	if ${github-release} info --tag $TAG > /dev/null 2>&1 ; then
		...
		`sed -i "" -e "1,/Major:.*/s/Major:.*/Major: $MAJOR,/" \
			...
			-e "1,/Build:.*/s/Build:.*/Build: $BUILD,/" cf-inspect.go`
	fi
fi

LINUX_FILENAME="inspect-plugin-linux"
MAC_FILENAME="inspect-plugin-darwin"
WIN_FILENAME="inspect-plugin.exe"
...
#    --description "$TAG release - work in progress"
    ...
    --tag $TAG \
    --name "inspect-plugin-darwin" \
    --file bin/osx/inspect-plugin-darwin

	${github_release} upload \
    --tag $TAG \
    --name "inspect-plugin-linux" \
    --file bin/linux64/inspect-plugin-linux

	${github_release} upload \
    --tag $TAG \
    --name "inspect-plugin.exe" \
